...
local gameStarted, keys = false, {0, 1, 2, 3, 32}
-- broom={left={img="151c067545f.png",x=-38,y=-42},right={img="151c067ad92.png",x=-38,y=-42}
...
eventKeyboard = function(name, key, down, x, y)
    if gameStarted then
        ...
        elseif key == 0 then
            if down then
                tfm.exec.movePlayer(name, 0, 0, false, -50, 0, false)
            else
                tfm.exec.movePlayer(name, 0, 0, false, -1, -1, false)
                tfm.exec.movePlayer(name, 0, 0, false, 1, 1, true)
            end
    ...
    if players[name].playing and key == 32 then
        if players[name].cooldown == 0 then
            ...
            if players[name][3] == 0 or players[name][3] == 2 then
                players[name].lastObj = tfm.exec.addShamanObject(17, players[name][3] == 0 and x - 25 or x + 25, y,
                                            players[name][3] == 0 and -90 or 90, players[name][3] == 0 and -10 or 10, 0)
            else
                players[name].lastObj = tfm.exec.addShamanObject(17, x, players[name][3] == 1 and y - 25 or y + 25,
                                            players[name][3] == 1 and 0 or 180, 0, players[name][3] == 1 and -10 or 10)
            end
