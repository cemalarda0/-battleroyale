...
local gameStarted, bonusTime, keys = false, 0, {0, 1, 2, 3, 32, 80}
local textAreaIds = {
    aliveMice = 99,
    gift = {
        backGround = 100,
...
for _, i in next,
    {'AutoShaman', 'MinimalistMode', 'WatchCommand', 'MortCommand', 'AutoNewGame', 'AutoScore',
...
addBonusImg = function(event, name)
    ...
    if event == 1 or event == 2 then
        ...
        players[name].event.img = tfm.exec.addImage("16fe31c49aa.png", "&100", 765, 365, name, 1, 1, math.rad(event == 1 and 0 or 180), 1,
                                      0.5, 0.5)
    ui.addTextArea(textAreaIds.gift.onImg, "\n\n<p align ='right'><font color ='"..color.."'>5</font></p>", name, 745, 352, 50, 50, nil, nil, 0, true)
    elseif event == 3 then
        players[name].event.img = tfm.exec.addImage("16fe31c8ffd.png", "&100", 765, 365, name, 1, 1, math.rad(0), 1,
                                      0.5, 0.5)
    elseif event == 4 or event == 5 then
        players[name].event.img = tfm.exec.addImage("1752b1c10bc.png", "&100", 765, 363, name, 1.05, 1.05, math.rad(0), 1,
                                      0.5, 0.5)
        ui.addTextArea(textAreaIds.gift.onImg, "\n\n<p align ='right'><font color ='#ff0000'>x"..players[name].obj.count.."</font></p>", name, 740, 348, 50, 50, nil, nil, 0, true)
    end
end
updatePlaying = function(name)
    ...
    ui.addTextArea(textAreaIds.aliveMice, "<p align='right'><font size='25'><font color='"..color.."'>".. getPlaying().."</font></font></p>", name, 20, 350, 70, 40, 0xFFDDAA, 0x000000, 1, true)
